...
// useEffect 훅 : 사이드 이펙트 관리 훅
// 함수형 컴퍼넌트의 생명주기 함수를 지원한다.
// 1.마운트 : useEffect( ()=>{}, [] );
// 2.언마운트 : useEffect(  ()=>{ return()=>{},[] } );
// 3.업데이트(props,state가 업데이트될 때) : useEffect( ()=>{} , [state1,state2] );
...
// useMeno 훅
// useMeno는 특정 연산의 결과를 메모이제이션(저장)하여
// 불필요한 재계산을 방지하는데 사용한다.
// 컴퍼넌트가 랜더링 될 때 마다 매번 계산하는 대신
...
export function Counter4() {
  ...
  // number 상태변수가 변경되지 않는한,double값은 재연산되지 않음.
...
export function Counter5() {
  ...
  // useCalback으로 숫자 입력 핸들러(콜백함수) 메모이제이션
  // e는 JS 이벤트 객체,이벤트 함수에서 전달됨.
...
export function Counter6() {
  // clickCountRef는 컴퍼넌트가 리렌더링 될 때 값이 초기화 되지 않는다.
  ...
  const handleClick = () => {
    setCount(count + 1);
    clickCountRef.current += 1; // A = A + B , A += B
...
export function Counter7() {
  ...
  const handleClick = () => {
    setCount(count + 1); //상태 변경은 리액트에서 비동기적으로 이루어지므로, 바로 +1이 안됨.
  ...
  return (
    <div>
      ...
      <input type="text" ref={inputRef} placeholder="여기에 입력하시오." />
      <br />

...
INSERT INTO 학생 
VALUES ('S0001', '강감찬', '2000-02-03', '01022223333', '03', '남', NULL
INSERT INTO 학생 
...
USE 세계무역;
DESC 제품;
ALTER TABLE 제품 ADD CONSTRAINT chk_재고 CHECK (재고 >= 0);
-- ALTER TABLE 제품 DROP CONSTRAINT chk_재고;
...
--     * ALTER TABLE 명령을 사용합니다.
--     * 외래키 제약조건을 설정할 때  ON DELETE CASCADE 옵션을 설명하면, 부모 레코드 삭제시 자식 레코드도 연쇄적으로 삭제되어 데이터의 일관성을 유지할 수 있습니다.
ALTER TABLE 주문세부 
ADD CONSTRAINT fk_제품번호 
FOREIGN KEY (제품번호) REFERENCES 제품(제품번호) 
ON DELETE CASCADE;

-- 영화 테이블과 평점관리 테이블을 만들고자 합니다. 다음 테이블 명세서를 참고하여 테이블을 생성하시오.
CREATE DATABASE IF NOT EXISTS 연습문제;
USE 연습문제;
CREATE TABLE 영화 (
...
DESC 평점관리;
-- 다음 테이블 명세서를 참고하여 평점관리 테이블을 생성하시오.
CREATE TABLE 평점관리 (
